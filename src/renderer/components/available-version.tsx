  ...
  return (props: T) => {
    for (const variant of variants) {
      try {
        ...
        if (store) {
          Common.logger.debug(
            `[@borakostem/llm-companion]: Rendering ${resourceName} page with API version ${variant.version}`,
          );
          return <variant.PageComponent {...props} />;
        }
      } catch (error) {
        Common.logger.debug(
          `[@borakostem/llm-companion]: API version ${variant.version} not available for ${resourceName}: ${error}`,
        );
      }
    }
    ...
    Common.logger.info(
      `[@borakostem/llm-companion]: ${resourceName} CRD not found in cluster (tried versions: ${triedVersions})`,
    );
